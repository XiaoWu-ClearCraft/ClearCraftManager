#!/bin/bash
# ClearCraftManager installation script.
# This script installs or updates the ClearCraftManager Web and/or Daemon to the latest version.
# ClearCraftManager is a customized panel based on MCSManager.
# ------------------------------------------------------------------------------
...
download_fallback_url="https://gh.xmly.dev/https://github.com/XiaoWu-ClearCraft/ClearCraftManager/releases/latest/download/clearcraftmanager_linux_release.tar.gz"

# Name of the release package to download/detect
package_name="clearcraftmanager_linux_release.tar.gz"
...
# Node.js installation path (defaults to the ClearCraftManager installation path. Can be overridden with --node-install-dir)
...
cprint() {
  ...
  if [[ "$disable_prefix" != true ]]; then
    ...
    local label="[ClearCraft Installer]"
...
download_mcsm() {
  ...
  cprint cyan bold "Downloading ClearCraftManager package..."
  ...
  mv "$extracted_tmp_path" "$install_tmp_dir" || {
    ...
    cprint green bold "ClearCraftManager source extracted and moved to: $install_tmp_dir"
  return 0
}
...
install_component() {
  ...
  local source_path="${install_tmp_dir}/clearcraftmanager/${component}"
...
[Unit]
Description=ClearCraftManager-${component^}
...
print_install_result() {
  ...
  # Official doc
  cprint yellow noprefix  "Project Repository:"
  cprint white noprefix "  https://github.com/XiaoWu-ClearCraft/ClearCraftManager"
  echo ""

  # ClearCraftManager disclaimer
  cprint yellow noprefix  "Disclaimer:"
  cprint white noprefix "  ClearCraftManager is a customized panel based on MCSManager."
  cprint white noprefix "  It is NOT affiliated with the MCSManager official team."
  echo ""
  ...
  # Closing message
    cprint green noprefix "Installation completed. Enjoy managing your servers with ClearCraftManager!"
  echo ""
}
...
main() {
  ...
  safe_run download_mcsm "Failed to acquire ClearCraftManager source"
  safe_run mcsm_install_prepare "Error while preparing for installation"
  
  safe_run install_mcsm "Failed to install ClearCraftManager"
}
main "$@"